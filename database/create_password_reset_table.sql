...
-- This table manages password reset tokens and requests
...
CREATE TABLE IF NOT EXISTS password_reset_requests (
    ...
    created_by UUID REFERENCES auth.users(id), -- Admin who created the request
    used_by UUID REFERENCES auth.users(id) -- User who used the reset
);
...
CREATE OR REPLACE VIEW v_password_reset_requests_with_users AS
SELECT 
    prr.*,
    u.full_name,
...
SELECT 
    id,
    email,
    'sample_token_' || substr(md5(random()::text), 1, 20),
    'email',
    'pending',
...
-- Insert some expired sample data
INSERT INTO password_reset_requests (user_id, email, token, reset_type, status, expires_at, created_by)
SELECT 
    id,
    email,
    'expired_token_' || substr(md5(random()::text), 1, 20),
    'email',
    'expired',
    NOW() - INTERVAL '1 hour',
    id
